<template>
    <tr>
        ...
        <td scope="">
            {{ props.content.num_hours }}
        </td>
        <td scope="">
            <!-- {{ props.content.drops }} -->
            <ul class="dropStyle">
                <span v-for="(dropInfo, index) in props.content.drops" :key="index" class="dropsInfo">
                    <img :src="dropInfo.img_url" alt=""/>
                        {{ dropInfo.drop_name }} : {{ dropInfo.drop_quantity }}
                </span>
            </ul>
        </td>
        <td scope="">
...
<script setup>
    ...
    const deleteDrop = () => {
        emits('delete', props.content.id_hours)
    }
